...
txt_path_training = "experiments/ADMOD_vs_HC/interset/spectral/splits/training_subjects.txt"
txt_path_validation = "experiments/ADMOD_vs_HC/interset/spectral/splits/validation_subjects.txt"

with open(txt_path_training, "r") as f:
    num_lines_training = sum(1 for _ in f)
with open(txt_path_validation, "r") as f:
    num_lines_validation = sum(1 for _ in f)

logger.info(f"Lines in training_subjects.txt: {num_lines_training}")
logger.info(f"Lines in validation_subjects.txt: {num_lines_validation}")

training_dataset = SpectralDataset(
    h5_file_path=h5_file_path, 
    subjects_txt_path="experiments/ADMOD_vs_HC/interset/spectral/splits/training_subjects.txt",
    normalize=config.scaler_type
)

validation_dataset = SpectralDataset(
    h5_file_path=h5_file_path, 
    subjects_txt_path="experiments/ADMOD_vs_HC/interset/spectral/splits/validation_subjects.txt",
    normalize=config.scaler_type
)
