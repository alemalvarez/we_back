...
def main():
    ...
    # Initialize wandb
    wandb.init(project="ADMOD_vs_HC_sweep", config=vars(args))
    config = wandb.config
    ...
    training_dataset = SpectralDataset(
        ...
        subjects_txt_path="experiments/ADMOD_vs_HC/POCTEP/spectral/splits/training_subjects.txt",
        normalize="standard"
    )

    validation_dataset = SpectralDataset(
        h5_file_path="artifacts/POCTEP_DK_features_only:v0/POCTEP_DK_features_only.h5",
        subjects_txt_path="experiments/ADMOD_vs_HC/POCTEP/spectral/splits/validation_subjects.txt",
        normalize="standard"
    )
